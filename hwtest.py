...
You should test on a platform with valgrind before turning this in!" + TextColors.END)
    ...
    def execute(self, add_valgrind=False, suppress_output=False):
        ...
        If add_valgrind is specifically "NOLEAK", then leak checks will be disabled, but we'll still check for memory misuse.
        ...
        if has_valgrind and add_valgrind:
            # These options force an exit with special exit status on valgrind error (leak or pointer misuse)
            # The --exit-on-first-error=yes prevents valgrind itself from choking on a corrupted heap, which makes it not give the error-exitcode requested
            # This doesn't cause spurious output-differs feedback because the valgrind run is separate from the regular correctness run
            if add_valgrind=="NOLEAK":
                leak_option = "no"
            else: # add_valgrind is generically True, so do the leak check
                leak_option = "full"
            command_argv = ["valgrind","-q","--exit-on-first-error=yes","--error-exitcode=%d" % EXITCODE_VALGRIND_ERROR,"--show-reachable=yes","--leak-check="+leak_option] + command_argv
    ...
    def run(self, generate_expected=False):
        """
        ...
        if generate_expected:
            return TestResult(test=self, is_pass=True, points=max_points, message="Blessed", error_flags=error_flags)
        ...
        if self.has("penalty_valgrind"):
            ...
            if exitcode_with_valgrind==EXITCODE_VALGRIND_ERROR:
                ...
                message += "Valgrind memory error or leak detected! (Test score will be multiplied by %.2f)\n" % (self["penalty_valgrind"])
        elif self.has("penalty_valgrind_noleak"):
            exitcode_with_valgrind = self.execute(add_valgrind="NOLEAK", suppress_output=True)
            if exitcode_with_valgrind==EXITCODE_VALGRIND_ERROR:
                is_pass = False
                penalty *= self["penalty_valgrind_noleak"]
                error_flags.append("valgrind_error")
                message += "Valgrind memory error detected! (Test score will be multiplied by %.2f)\n" % (self["penalty_valgrind_noleak"])
...
class Suite(JSONWrapper):
    ...
    def run(self, generate_expected=False):
        ...
        for test in self.tests:
            try:
                result = test.run(generate_expected=generate_expected)
...
class Tester(JSONWrapper):
    ...
    def run_suites(self, suite_names, generate_expected=False):
        """
        ...
        for suite in self.each_suite(suite_names):
            test_result_set += suite.run(generate_expected=generate_expected)
...
def main():
    ...
    test_result_set = tester.run_suites(suite_names,generate_expected=args.generate_expected)
